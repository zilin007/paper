...
从 arXiv 搜索论文、下载 PDF、提取原图，并生成通俗易懂的中文 Markdown 解读（含 LaTeX 公式、数值推导、原图引用）。
...
| Python 标准库 | arXiv API 搜索与 PDF 下载 | `download_arxiv.py` 无需任何第三方依赖 |
...
推荐使用 [Obsidian](https://obsidian.md)（免费）打开本仓库目录作为 Vault，原生支持 LaTeX 渲染和图片预览。
