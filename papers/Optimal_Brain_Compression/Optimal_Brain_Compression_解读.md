...
3. **OBC (Optimal Brain Compressor)统一框架**: ExactOBS和OBQ可以交替使用,实现剪枝和量化的协同优化
...
1. 先用ExactOBS剪枝$w_{13} \to 0$
2. 再用OBQ量化$w_{11} \to 1$
3. 继续剪枝$w_{21} \to 0$
4. 继续量化其他权重...

每一步都使用相同的Hessian逆矩阵更新公式,保证误差最小化。这就是**统一框架**的核心优势:剪枝和量化共享同一套数学工具,可以灵活组合。
