...
流程推导必须**一步一步连贯推导，禁止跳跃或黑盒式给出结果**。具体规则：

- **损失函数计算**：如果损失由多部分组成（如重建损失 + 正则化损失），必须**分别计算每一部分**，展示中间数值，再求和。禁止直接给出总损失。
- **梯度推导**：涉及梯度计算时，必须展示**链式法则的完整路径**。对每个参数，写出：
  1. 梯度的数学表达式（如 $\frac{\partial L}{\partial V_0} = \frac{\partial L_{recon}}{\partial V_0} + \frac{\partial L_{reg}}{\partial V_0}$）
  2. 每一项的具体数值计算过程（代入数字，展示中间步骤）
  3. 最终梯度向量
  禁止只写"计算梯度得 [0.3, 0.15, 0.19]"而不展示推导过程。
- **参数更新**：必须展示更新公式（如 $V^{(1)} = V^{(0)} - \eta \nabla_V L$），代入具体学习率和梯度值，算出更新后的参数。禁止直接给出更新后的值。
- **迭代过程**：如果算法有多次迭代，用**表格**清晰展示每一步的中间状态（如 V 值、h 值、损失值），让读者能看到收敛过程。
- **解释局限性**：如果简化示例（如单样本）的结果与真实场景（多样本）有差异，必须明确解释原因，避免读者困惑。
